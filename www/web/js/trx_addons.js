    ...
    function trx_addons_ready_actions(e, container) {
        ...
        if (TRX_ADDONS_STORAGE['animate_inner_links'] > 0 && !container.hasClass('animate_to_inited')) {
            container.addClass('animate_to_inited').on('click', 'a', function(e) {
                ...
                if (link_parent.parent().hasClass('trx_addons_tabs_titles') || link_obj.hasClass('trx_addons_panel_link') || link_obj.hasClass('trx_addons_popup_link') || link_parent.hasClass('vc_tta-tab') || link_obj.hasClass('vc_pagination-trigger') || link_obj.hasClass('ui-tabs-anchor') || link_parent.hasClass('vc_tta-panel-title') || link_parent.hasClass('wpb_accordion_header') || link_parent.parents('.wc-tabs').length > 0 || link_parent.parents('ul[class*="tabs"]').length > 0) return;
        ...
        if (TRX_ADDONS_STORAGE['popup_engine'] == 'pretty') {
            ...
            var images = container.find("a[rel*='prettyPhoto']:not(.inited)" + ":not(.esgbox)" + ":not([data-rel*='pretty'])" + ":not([rel*='magnific'])" + ":not([data-rel*='magnific'])" + ":not([data-elementor-open-lightbox='yes'])" + ":not([data-elementor-open-lightbox='default'])").addClass('inited');
            try {
                images.prettyPhoto({
                    social_tools: '',
                    theme: 'facebook',
            ...
            var images = container.find("a[rel*='magnific']:not(.inited)" + ":not(.esgbox)" + ":not(.prettyphoto)" + ":not([rel*='pretty'])" + ":not([data-rel*='pretty'])" + ":not([data-elementor-open-lightbox='yes'])" + ":not([data-elementor-open-lightbox='default'])").addClass('inited');
            setTimeout(function() {
            ...
            container.find('.sc_layouts_popup:not(.inited),.sc_layouts_panel:not(.inited)').each(function() {
                ...
                jQuery('a[href="#' + id + '"]').addClass(is_panel ? 'trx_addons_panel_link' : 'trx_addons_popup_link').data('panel', obj);
    ...
    window.trx_addons_debug_object = function(obj) {
        ...
        if (obj !== null && (typeof(obj) == 'object' || typeof(obj) == 'array' || (typeof(obj) == 'function' && level == 0))) {
            for (var prop in obj) {
                ...
                if (level < recursive && (typeof(obj[prop]) == 'object' || typeof(obj[prop]) == 'array') && obj[prop] != obj) dispStr += addStr + prop + '=' + trx_addons_debug_object(obj[prop], recursive, showMethods, level + 1);
                else {
                    try {
                        curStr = "" + obj[prop];
                    } catch (e) {
                ...
                jQuery.post(TRX_ADDONS_STORAGE['ajax_url'], {
                    ...
                    if (rez.error === '') {
                        ...
                        if (load_more) {
                            ...
                            if (link.data('page') >= link.data('max-page')) pagination_wrap.fadeOut();
                            else link.data('page', Number(link.data('page')) + 1);
                        } else {
                            sc.fadeOut(function() {
            ...
            container.find('.sc_slider_controls:not(.inited)').each(function() {
                ...
                if (pagination_style == 'thumbs') {
                    ...
                    slides.each(function() {
                        var slide = jQuery(this);
                        var image = slide.data('image');
                        html += '<span class="slider_pagination_button_wrap swiper-pagination-button-wrap" style="width: ' + Math.round(100 / slides.length, 2) + '%;">' + '<span class="slider_pagination_button swiper-pagination-button"' + (image !== undefined ? ' style="background-image: url(' + image + ');"' : '') + '></span>' + '</span>';
                    });
                    controls.find('.slider_pagination_wrap').html(html);
                } else if (pagination_style == 'fraction') {
                    controls.find('.slider_pagination_wrap').html('<span class="slider_pagination_current swiper-pagination-current">1</span>' + '/' + '<span class="slider_pagination_total swiper-pagination-total">' + jQuery('#' + slider_id + ' .swiper-slide').length + '</span>');
...
jQuery(document).on('action.ready_trx_addons', function() {
    ...
    if (jQuery('.sc_layouts_cart').length > 0 && !jQuery('body').hasClass('added_to_cart_inited')) {
        ...
        jQuery('.sc_layouts_cart:not(.inited)').addClass('inited').on('click', '.sc_layouts_cart_icon,.sc_layouts_cart_details', function(e) {
            ...
            console.log(widget.length, widget.text().replace(/\s*/g, ''));
